            ...
            <div class="col-xs-12">
                ...
                <div class="box">
                    <div class="box-header">
                        <div class="box-header">
                            <h3 class="box-title">Edit Page</h3><a class="btn btn-small btn-primary pull-right" href="{{ route('pages.index') }}"><i class="fa fa-list"></i>&nbsp; List</a>
                        </div>
                    </div>
                </div>
                <div class="box">
                    ...
                    <div class="box-body">
                                    ...
                                    <h4 class="tag-category">{{$value->name}}</h4>
                                        <ul class="no-list-style">
                                            @foreach($value->tag as $avalue)
                                                <li>{!! Form::checkbox('tag_ids[]', $avalue->id, $atag_ids) !!} {{$avalue->name}}</li>
                                            @endforeach
                                        </ul>
                                 </div>
                            </div>
                        ...
                        <div class="col-md-12">
                            ...
                            @for($i=1; $i<=3; $i++)
                            <?php
                            $add_image = isset($add_images['add_image_' . $i]) ? $add_images['add_image_' . $i] : null;
                            $add_image_name = isset($add_images['add_image_name_' . $i]) ? $add_images['add_image_name_' . $i] : null;
                            $add_image_alt_tag = isset($add_images['add_image_alt_tag_' . $i]) ? $add_images['add_image_alt_tag_' . $i] : null;
                            $add_image_link = isset($add_images['add_image_link_' . $i]) ? $add_images['add_image_link_' . $i] : null;
                            ?>
                            <h4>Advert {{$i}}</h4>
                            <div class="form-group">
                                {!! Form::label('add_image_'.$i, 'Add Image', ['class'=>'text-right']) !!}
                                {!! Form::file('add_image_'.$i, null, array('class' => 'form-control')) !!}
                                <img src="{{asset($add_image)}}" alt="" height="30" />
                                <input type="hidden" name="hidden_add_image_{{$i}}" value="{{$add_image}}">
                            </div>
                            <div class="form-group">
                                {!! Form::label('add_image_name_'.$i, 'Add Image Name', ['class'=>'text-right']) !!}
                                {!! Form::text('add_image_name_'.$i, $add_image_name, array('class' => 'form-control')) !!}
                            </div>
                            <div class="form-group">
                                {!! Form::label('add_image_alt_tag_'.$i, 'Add Image alt tag', ['class'=>'text-right']) !!}
                                {!! Form::text('add_image_alt_tag_'.$i, $add_image_alt_tag, array('class' => 'form-control')) !!}
                            </div>
                            <div class="form-group">
                                {!! Form::label('add_image_link_'.$i, 'Add Image link', ['class'=>'text-right']) !!}
                                {!! Form::text('add_image_link_'.$i, $add_image_link, array('class' => 'form-control')) !!}
                            </div>
                            @endfor
                            ...
                            <div class="form-group">
                                ...
                                @php
                                $media = $page->getMedia('page_thumb')->first();
                                $bmedia_name = isset($media->name) ? $media->name : "";
                                @endphp
                                @if(!empty($media))
                                <img src="{{$media->getUrl()}}" alt="" height="30" />
                                @endif
                            </div>
                            <div class="form-group">
                                {!! Form::label('banner_image_name', 'Banner Image Name', ['class'=>'text-right']) !!}
                                {!! Form::text('banner_image_name', $bmedia_name, array('class' => 'form-control')) !!}
                            </div>
                            <div class="form-group">
